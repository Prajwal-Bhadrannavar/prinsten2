...
exports.getHealthInsights = exports.getLifestyleData = exports.saveLifestyleData = void 0;
const LifestyleData_1 = __importDefault(require("../models/LifestyleData"));
...
exports.saveLifestyleData = saveLifestyleData;
...
const getHealthInsights = async (req, res) => {
    try {
        ...
        const avgSteps = Math.round(lifestyleData.reduce((sum, data) => sum + data.steps, 0) / lifestyleData.length);
        const avgWater = Math.round(lifestyleData.reduce((sum, data) => sum + data.waterIntake, 0) / lifestyleData.length);
        const avgSleep = Math.round(lifestyleData.reduce((sum, data) => sum + data.sleepHours, 0) / lifestyleData.length);
        const avgExercise = Math.round(lifestyleData.reduce((sum, data) => sum + data.exerciseMinutes, 0) / lifestyleData.length);
        const insights = [];
        if (avgSteps < 8000) {
